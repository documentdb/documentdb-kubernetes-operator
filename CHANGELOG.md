...
### Major Features
- **Default DocumentDB version bumped to `0.113.0`**: the default documentdb extension and gateway images now default to `0.113.0` (was `0.110.0`), so installing the latest operator Helm chart ships the latest supported database images. In-place upgrades from `0.110.0` are handled via the two-phase extension upgrade flow below.
