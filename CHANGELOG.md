...
- **Container metrics via chart-managed DaemonSet collector**: The Helm chart now ships an opt-in OpenTelemetry Collector DaemonSet (`containerMetrics.enabled=true`, default `false`) that scrapes each node's local kubelet for container, pod, and node CPU/memory/network/filesystem metrics and exposes them via Prometheus. The DaemonSet uses a single chart-managed ServiceAccount with `nodes/stats` GET privilege; tenant DocumentDB clusters do not receive any kubelet privileges, and the operator no longer manages kubeletstats-specific cluster-scoped RBAC. Matches OpenTelemetry's recommended deployment pattern for the kubeletstats receiver.

### Breaking Changes
- **Removed `spec.monitoring.kubeletstats` (preview API)**: The per-`DocumentDB` toggle for kubelet-sourced container metrics has been removed. Container resource metrics are now collected by the chart-managed `containerMetrics` DaemonSet (see Major Features). This removes the per-tenant `nodes/stats` privilege grant, removes the kubeletstats-specific `bind` permission added for the earlier sidecar design, and stops injecting `K8S_NODE_NAME` into the sidecar. Migration: drop `spec.monitoring.kubeletstats: {}` from your `DocumentDB` resources and install/upgrade the operator chart with `--set containerMetrics.enabled=true`.
