...
### Major Features
- **io_uring opt-in feature gate**: A new `IOUring` feature gate (`spec.featureGates.IOUring: true`) enables PostgreSQL 18 asynchronous I/O (`io_method=io_uring`). Because the container runtime's default seccomp profile strips the `io_uring_setup/enter/register` syscalls, the operator also relaxes the postgres container seccomp profile when the gate is on — no external Kyverno policy needed. It is **opt-in** (default off) since io_uring relaxes the sandbox. Seccomp behavior is operator-level config via Helm values `operator.ioUring.seccompMode` (`localhost` hardened default, or `unconfined`) and `operator.ioUring.seccompProfile`. See [io_uring documentation](docs/operator-public-documentation/io-uring.md) and the [feature playground](documentdb-playground/io-uring-feature/).
