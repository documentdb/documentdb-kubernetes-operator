...
var _ = Describe("DocumentDB scale — up",
	...
	func() {
		...
		It("scales 2 → 3 instances while keeping the primary pod stable", func() {
			...
			Expect(shareddb.PatchInstances(ctx, c, key.Namespace, key.Name, 3)).To(Succeed())
		...
		It("scales 1 → 2 instances after first scaling down to 1", func() {
			...
			Expect(shareddb.PatchInstances(ctx, c, key.Namespace, key.Name, 1)).To(Succeed())
			...
			Expect(shareddb.PatchInstances(ctx, c, key.Namespace, key.Name, 2)).To(Succeed())
