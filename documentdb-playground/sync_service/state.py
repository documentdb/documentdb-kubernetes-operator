        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, 'r') as f:
                    ...
                    # Migrate old single-token format to per-collection format
                    if "resume_token" in self._state and "resume_tokens" not in self._state:
                        self._state["resume_tokens"] = {}
                        del self._state["resume_token"]
