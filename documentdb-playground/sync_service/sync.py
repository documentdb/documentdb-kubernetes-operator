...
Usage:
    ...
    --reset: Reset sync state and start fresh (WARNING: may cause duplicates)
"""
...
def generate_state_file_path(source_uri: str, target_uri: str) -> str:
    ...
    This ensures different sync pairs use different state files.
    """
    source_name = extract_cluster_name(source_uri)
    target_name = extract_cluster_name(target_uri)
    
    return f"~/.documentdb_sync_state_{source_name}_to_{target_name}.json"
...
def sync_change(
    ...
    try:
        if operation == "insert":
            ...
            if full_document:
                # Remove _id from document for replace (Azure DocumentDB doesn't allow _id in replacement doc)
                doc_to_write = {k: v for k, v in full_document.items() if k != "_id"}
                # Get document ID - handle different key formats (local DocumentDB uses "" key)
                doc_id = document_key.get("_id") or document_key.get("") or full_document.get("_id")
                if not doc_id and document_key:
                    doc_id = list(document_key.values())[0]
            ...
            # Get document ID - handle different key formats
            doc_id = document_key.get("_id") or document_key.get("") or list(document_key.values())[0] if document_key else None
            if doc_id:
