...
> **⚠️ IMPORTANT:** AKS Edge runs K3s 1.31. Newer operator versions may require K8s 1.35+.
...
# Install operator from public OCI registry (v0.1.3 works with K3s 1.31)
...
> CloudNative-PG validates image tags. Tags must be simple PostgreSQL versions like `:17`, `:16`, `:17.0`.
...
# (Optional) Create image pull secret for private registries
# Uncomment if using custom images from private ghcr.io repos
...
# Gateway image includes IPv4 fallback fix for environments without IPv6
@\"
...
**Wait for:** Pod `demo-documentdb-1` to show `2/2 Running`
...
**Success check:** Status shows \"Cluster in healthy state\" and both containers are running

**Verify IPv4 binding (if gateway container is running):**
```powershell
kubectl logs demo-documentdb-1 -n app-namespace -c documentdb-gateway | Select-String \"Bound\"
# Should show: \"Bound to IPv4 address 0.0.0.0:10260\"
```

---

### Phase 8: Verify DocumentDB Connection

**Tell user:** \"Now let's verify DocumentDB is working by connecting and inserting data\"

#### Step 1: Port Forward (Terminal 1)

```powershell
# Keep this running - opens connection to DocumentDB gateway
kubectl port-forward svc/documentdb-service-demo-documentdb 10260:10260 -n app-namespace
```

#### Step 2: Get Credentials (Terminal 2)
...
```powershell
# Connect using credentials from Step 2
...
#### Step 5: Insert and Query Documents (in mongosh)
...
**Success check:** Documents are inserted and queried successfully. Expected output:
```
...
### Phase 9: Create Portal Access Token
...
6. Paste the token from Phase 9
...
| ImagePullBackOff for private images | Create image pull secret (see Phase 7) |
...
| CNPG \"invalid version tag\" | Retag image with simple version (e.g., `:17` not `:pg17-0.111.0`) |
| CNPG \"Unsupported PostgreSQL version\" | Use tag format like `:17`, `:16`, `:17.0` |
| Operator K8s version check fails | Use operator v0.1.3 which doesn't require K8s 1.35 |
| Gateway CrashLoopBackOff (IPv6) | Use gateway image v0.109.0+ with IPv4 fallback |
...
| 8. Verify connection | Any (2 terminals) | `db.users.find()` returns inserted documents |
| 9. Portal token | Any | Token output saved |
| 10. Portal view | Browser | See pods in Azure Portal |
