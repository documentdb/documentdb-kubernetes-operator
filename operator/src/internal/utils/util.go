...
// KubeletStatsClusterRoleName is the cluster-scoped Role granting GET on
// nodes/stats and nodes/proxy. Installed by the operator's Helm chart and
// referenced by per-cluster ClusterRoleBindings created by the operator
// when a DocumentDB has spec.monitoring.kubeletstats set.
const KubeletStatsClusterRoleName = "documentdb-kubeletstats-reader"

// Labels applied to per-cluster kubeletstats ClusterRoleBindings so they
// can be discovered and bulk-cleaned (e.g., by a Helm pre-delete hook in
// future) and so audit tooling can map a CRB back to its DocumentDB CR.
const (
	KubeletStatsManagedByLabel = "app.kubernetes.io/managed-by"
	KubeletStatsManagedByValue = "documentdb-operator"
	KubeletStatsClusterLabel   = "documentdb.io/cluster"
	KubeletStatsNamespaceLabel = "documentdb.io/namespace"
	KubeletStatsComponentLabel = "documentdb.io/component"
	KubeletStatsComponentValue = "kubeletstats"
)

// KubeletStatsClusterRoleBindingName returns the deterministic name of the
// per-cluster ClusterRoleBinding. We hash the namespace+name pair to (a)
// avoid collisions with hyphens (e.g. "a-b/c" vs "a/b-c") and (b) stay
// well within the 253-char limit while keeping a recognisable prefix.
func KubeletStatsClusterRoleBindingName(name, namespace string) string {
	sum := sha256.Sum256([]byte(namespace + "/" + name))
	return "documentdb-kubeletstats-" + hex.EncodeToString(sum[:8])
}

// CreateOrUpdateKubeletStatsClusterRoleBinding ensures the per-cluster
// ClusterRoleBinding exists with the correct subject, RoleRef, and labels.
// Reconciles drift on every call (recreates the CRB if the immutable
// RoleRef has been tampered with).
func CreateOrUpdateKubeletStatsClusterRoleBinding(ctx context.Context, c client.Client, clusterName, namespace string) error {
	bindingName := KubeletStatsClusterRoleBindingName(clusterName, namespace)
	desiredLabels := map[string]string{
		KubeletStatsManagedByLabel: KubeletStatsManagedByValue,
		KubeletStatsComponentLabel: KubeletStatsComponentValue,
		KubeletStatsClusterLabel:   clusterName,
		KubeletStatsNamespaceLabel: namespace,
	}
	desiredSubjects := []rbacv1.Subject{
		{Kind: "ServiceAccount", Name: clusterName, Namespace: namespace},
	}
	desiredRoleRef := rbacv1.RoleRef{
		Kind:     "ClusterRole",
		Name:     KubeletStatsClusterRoleName,
		APIGroup: "rbac.authorization.k8s.io",
	}

	found := &rbacv1.ClusterRoleBinding{}
	err := c.Get(ctx, types.NamespacedName{Name: bindingName}, found)
	if err != nil {
		if !errors.IsNotFound(err) {
			return err
		}
		binding := &rbacv1.ClusterRoleBinding{
			ObjectMeta: metav1.ObjectMeta{Name: bindingName, Labels: desiredLabels},
	...
	// RoleRef is immutable; if it drifted, recreate.
	if found.RoleRef != desiredRoleRef {
		if err := c.Delete(ctx, found); err != nil && !errors.IsNotFound(err) {
			return err
		}
		binding := &rbacv1.ClusterRoleBinding{
			ObjectMeta: metav1.ObjectMeta{Name: bindingName, Labels: desiredLabels},
	...
	// Patch subjects/labels if drifted.
	needsUpdate := !subjectsEqual(found.Subjects, desiredSubjects) || !labelsContain(found.Labels, desiredLabels)
	if !needsUpdate {
		return nil
	}
	if found.Labels == nil {
	...
	found.Subjects = desiredSubjects
...
// DeleteKubeletStatsClusterRoleBinding removes the per-cluster
// ClusterRoleBinding. Safe to call even if the binding doesn't exist.
func DeleteKubeletStatsClusterRoleBinding(ctx context.Context, c client.Client, clusterName, namespace string) error {
	bindingName := KubeletStatsClusterRoleBindingName(clusterName, namespace)
	binding := &rbacv1.ClusterRoleBinding{}
	err := c.Get(ctx, types.NamespacedName{Name: bindingName}, binding)
	if err != nil {
		if errors.IsNotFound(err) {
			return nil
		}
	...
	if err := c.Delete(ctx, binding); err != nil && !errors.IsNotFound(err) {
		return err
	}
	return nil
}
